...
public abstract class PircBot implements ReplyConstants {
    ...
    public final synchronized void connect(String hostname, int port, String password) throws IOException, IrcException, NickAlreadyInUseException {
        ...
        // Connect to the server through proxy
        final Socket socket;
        ...
        if (proxyHost != null) {
            ...
            outputStreamWriter.write("CONNECT " + hostname + ":" + port + "\r\n");
        ...
        if (socket.isConnected()) {
            ...
            throw new RuntimeException("can't connect to " + hostname + ":" + port);
        }

        _inetAddress = socket.getLocalAddress();

        final InputStreamReader inputStreamReader = new InputStreamReader(socket.getInputStream(), getEncoding());
        ...
        if (proxyHost == null) {
            login(bwriter, nick, password);
        } else {
            OutputThread.sendRawLine(this, bwriter, "\r\n");
        }
        ...
        while ((line = breader.readLine()) != null) {
            ...
            if (proxyHost != null && line.contains("HTTP/1") && line.contains("200")) {
                login(bwriter, nick, password);
            }
    ...
    private void login(BufferedWriter bwriter, String nick, String password) {
        if (password != null && !password.equals("")) {
            OutputThread.sendRawLine(this, bwriter, "PASS " + password);
        }
        // connected so we can log now
        OutputThread.sendRawLine(this, bwriter, "NICK " + nick);
    ...
    public void log(String line) {
        if (_verbose) {
            System.out.println(DATE_FORMAT.format(new Date()) + " " + (line.startsWith(">>>")? ">>> " + line.substring(3) : line));
        }
    }
     ...
     * This method may not be overridden!
     *
     * @param raw The raw line of text from the server.
     */
    protected void handleLine(String raw) {
        String line = raw;
        this.log(line);
